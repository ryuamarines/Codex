...
OCR は `tesseract.js` ベースで、画像解析はブラウザ内で実行します。Tesseract の worker / core / 言語データは `public/tesseract` から同一オリジン配信し、OCR実行時に外部CDNへ取りに行かない構成です。チケットでは `日付 / 会場 / 開場 / 開演 / タイトル断片` を優先して補強し、立て看板では照合キーを補強する方向です。まずは `大量画像を一覧で整理して、人間が短時間で確定できる流れ` を優先しています。
