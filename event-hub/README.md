...
- API: ローカル開発専用 REST エンドポイント (`server.js` が `/api/session`, `/api/events`, `/api/reset`, `/api/export-csv`, `/api/import-csv` を処理)
- 配置 / 配信
...
- `/Users/Ryu/AI Workspace/Codex/event-hub/data/events.json`
- 初期化用シード: `/Users/Ryu/AI Workspace/Codex/event-hub/data/default-events.json`
...
```bash
cd /Users/Ryu/AI Workspace/Codex/event-hub
npm start
npm run test:smoke
```
...
- Vercel 公開版では `/api/*` を公開せず、`server.js` も `VERCEL=1` の環境では `/api/*` を 404 にします
- Firebase 設定は [public/src/firebase-config.js](/Users/Ryu/AI Workspace/Codex/event-hub/public/src/firebase-config.js:1) にあります
- Firestore ルールのたたき台は [firebase/firestore.rules](/Users/Ryu/AI Workspace/Codex/event-hub/firebase/firestore.rules:1) に置いています
- 現状 Firestore で触る path は `eventHub/appState` の 1 ドキュメントだけです
- Vercel 公開版では Google ログインを通してから Firestore を読む前提です
- 現在の運用では、URLを知っているチームメンバーが Google ログインすれば `eventHub/appState` を読めます
- さらに厳密にしたい場合は `public/src/firebase-config.js` の `FIRESTORE_ACCESS_MODE` を `member-doc` に戻し、`eventHubMembers/{uid}.active == true` 方式にします
- RoomPlaner のような `request.auth.uid == userId` は、`roomPlans/{userId}` のようなユーザー単位 path のときに使う設計です

### Firebase 認証 / Firestore の設定手順

1. Firebase Authentication で Google ログインを有効化する
2. Firestore Rules を [firebase/firestore.rules](/Users/Ryu/AI Workspace/Codex/event-hub/firebase/firestore.rules:1) の内容に更新する
3. URLを共有したいチームメンバーに Vercel 公開URLを渡す
...
- 保存窓口は [event-repository.js](/Users/Ryu/AI Workspace/Codex/event-hub/public/src/event-repository.js:1) にまとめています
- ローカルでは `server.js` 内の `/api` ハンドラを使い、`data/events.json` に保存します
- Vercel では `firestore` を使い、Firebase Web SDK から Cloud Firestore に保存します
- Vercel ではローカルJSON用の `/api/*` を無効化します
- 切り替えは [app-config.js](/Users/Ryu/AI Workspace/Codex/event-hub/public/src/app-config.js:1) で `localhost` かどうかを見て決めています
- CSV はローカル / Vercel のどちらでもバックアップ・受け渡し用に使えます
- Firestore 側は transaction で保存し、同時更新の衝突を減らしています

### クラウド同期の使い方

- クラウド同期したい場合は、Vercel 公開URLを開き、許可済み Google アカウントでログインします
- ログイン後の変更は `eventHub/appState` に自動保存され、同じメンバーが同じ共有データを見ます
- ローカル `http://127.0.0.1:3000` は `data/events.json` 保存なので、クラウドとは自動同期しません
- ローカルからクラウドへ移したい場合は、ローカルでCSV書き出し、Vercel側でCSV読込を使います
- 自分用の準備テンプレはブラウザに保存されます。チーム共通にしたいテンプレは標準テンプレとしてコードに追加します
...
- collection: `eventHub`
- document: `appState`
- フロント側の定義: [firebase-config.js](/Users/Ryu/AI Workspace/Codex/event-hub/public/src/firebase-config.js:11)
- 実際の読込 / 保存: [event-repository.js](/Users/Ryu/AI Workspace/Codex/event-hub/public/src/event-repository.js:121)

### 推奨 Rules

`event-hub` は現状、チームで共有する 1 ドキュメントを使っています。  
そのため、ユーザー単位の `request.auth.uid == userId` ではなく、Google ログイン済みかどうかで通します。
...
3. [firebase/firestore.rules](/Users/Ryu/AI Workspace/Codex/event-hub/firebase/firestore.rules:1) の内容を貼る
4. Publish する
...
- Google ログインは通るが保存できない
  - Rules が `allow read, write: if signedIn();` になっていない
